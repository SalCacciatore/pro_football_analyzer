...
def load_models():
    ...
    return {
        "rec_yardage": yardage_model,
        "rec_touchdown": touchdown_model,
        "pass_volume": pass_volume_model,
        "rush_yardage": rush_yards_model,
        "rush_touchdown": rush_touchdown_model,
    }
...
def process_rush_data(data, yardage_model, touchdown_model,szn):
    ...
    current_szn = rush_data[rush_data['season'] == szn]
...
#game_id = '2023_02_MIN_PHI'
    ...
    rush_touchdown_model = models["rush_touchdown"]
    rush_yardage_model = models["rush_yardage"]
...
# %%
    ...
    turnover_plays = all_plays[all_plays['turnover']==1][['posteam','qtr','desc','epa','wpa']]
...
# %%    
    ...
    pass_turnover_df = game[(game['turnover']==1)&(game['pass']==1)].groupby('posteam')[['epa']].sum().rename(columns={'epa':'pass_turnover_epa'})
    ...
    rushing_epa_df = game[game['rush']==1].groupby('posteam').agg({'epa':'sum'}).rename(columns={'epa':'designed_run_epa'})

    epa_df = passing_epa_df.merge(rushing_epa_df,right_index=True,left_index=True,how='outer')
...
# %%
    ...
    for i, idx in enumerate(passing_epa_df.index):
        fig5.add_trace(go.Bar(x=epa_df.columns, y=epa_df.loc[idx], name=str(idx)))
...
# %%
    ...
    rushers['success_rate'] = round(rushers['success']/rushers['rush'],3)
...
# Add a footnote
    footnote = "Higher is better for both offense and defense; a higher defensive PROE means opposing offenses have been more run-heavy<br>Data: nflverse<br>Chart: Sal Cacciatore<br>www.sportsandmaybeotherthings.com"
    fig.add_annotation(
        text=footnote,
        xref="paper", yref="paper",
        x=0, y=-0.18,
...
# Add a footnote
    footnote = "Higher is better for both offense and defense<br>Data: nflverse<br>Chart: Sal Cacciatore<br>www.sportsandmaybeotherthings.com"
    fig.add_annotation(
        text=footnote,
        xref="paper", yref="paper",
        x=0, y=-0.18,
...
with header:
    ...
    st.write("Play-by-play data, and EP, WP, CP and YAC models from NFLVerse. Other models are my own.")
...
def get_team_stats(team, year):
    ...
    year2 = get_off_stats(team,last_year_stats,'last',previous)
...
def receiver_simulator(chosen_team, spread, total, excluded_receiver1, excluded_receiver2, receiver_name,trailing_games,attempts_input,target_share_input, threshold):
    ...
    yardage_model = models["rec_yardage"]
    touchdown_model = models["rec_touchdown"]
    pass_volume_model = models["pass_volume"]
    ...
    'turnover': data['interception'] + data['fumble_lost'],
    'inside_10': (data['yardline_100'] < 10).astype(int),
    '20+_play': (data['yards_gained'] > 19).astype(int),
    'short_pass': (data['air_yards'] < 10).astype(int),
    'medium_pass': ((data['air_yards'] >= 10) & (data['air_yards'] < 20)).astype(int),
    'deep_pass': (data['air_yards'] >= 20).astype(int),
    'end_zone_target': (data['yardline_100'] - data['air_yards'] <= 0).astype(int),
    'distance_to_EZ_after_target': data['yardline_100'] - data['air_yards']
    })

# Drop rows with missing 'posteam' first to minimize unnecessary computation
    data = data.dropna(subset=['posteam'])
...
# Update the layout
    ...
    return team_rec_df, rec_df, receiver_string, median_yards, results[0], round(float(predicted_attempts),1), period_targets_per_game, szn_targets_per_game, rec_target_share, adot_yardage, adot_perc_above
