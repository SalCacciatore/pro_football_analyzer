...
# %%
    rushers = game[game['rush']==1].groupby('rusher_player_name').agg({'posteam':'max','rush':'sum','epa':'sum','success':'mean','yards_gained':'sum','turnover':'sum','touchdown':'sum','20+_play':'sum'}).round(2).sort_values(['posteam','rush'],ascending=False)
